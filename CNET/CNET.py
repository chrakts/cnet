...
class CNET(object):
   ...
   def sendCommand(self,command,expectAnswer = True):
      if type(self.interface)==str:
         boolAnswer,answer = self.sendCommandServer(command,expectAnswer)
      else:
         boolAnswer,answer = self.sendCommandTTY(command,expectAnswer)
      return(boolAnswer,answer)
            
   def sendCommandServer(self,command,expectAnswer = True):
